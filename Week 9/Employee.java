...
public class Employee
{
  ...
  /**
   * @return  The employee's salary as a floating-point number
   */
  ...
  public String getSupervisor() throws IOException
  {
    ...
    for (ArrayList<String> thisEmployee : employees)
    {
      ...
      {
        supervisor = thisEmployee.get(1).trim() + " " + thisEmployee.get(2).trim();
  ...
  /**
   * @param insurance  An integer value representing the option code for insurance status. Sets the insurance option
   *                   to 0 if the provided value is not expected.
   */
  ...
  /**
   * @param phone  A String as the employee's office phone number. Sets the number to "Unknown" if the provided value
  ...
  public void getEmployee() throws IOException
  {
    ...
    for (ArrayList<String> thisEmployee : employees)
    {
      // Assigning the value of the first column (the employee ID) to check against the ID passed to the current object
      int thisID = Integer.parseInt(thisEmployee.get(0).trim());
  ...
  public void printEmployee() throws IOException
  {
    ...
    System.out.printf("$ %9.2f  ", getSalary());
    System.out.printf("%-12s  ", this.status);
